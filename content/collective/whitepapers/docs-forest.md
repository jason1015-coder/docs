...
In scope as a project concern. The workflow runs inside the collective's GitHub Actions workspace, but the default deployment sends docs and code to a configured cloud model endpoint, so the material does leave the runner. That path is explicit configuration, never hidden behaviour, and every cloud call is scrubbed by the Prompt Scrubber and routed through the Private Inference Proxy. An operator who wants nothing to leave their own hardware configures a local model on a self hosted runner.
...
The substrate is the same whichever model is configured. The default is a cloud model on a GitHub hosted runner, called through the [Private Inference Proxy](/collective/whitepapers/private-inference-proxy) with the [Prompt Scrubber](/collective/whitepapers/prompt-scrubber) in front of it; an operator who runs a self hosted runner can point the same workflow at a local Llama instead.
...
- **Model configuration.** A configured cloud model on a GitHub hosted runner, the same posture ContentForest runs (ContentForest uses MiniMax), with every call passed through the Prompt Scrubber and the Private Inference Proxy. An operator who prefers local points the same configuration at a local provider on a self hosted runner.
...
The arrangement also tests the project's principles honestly. The model call goes to a configured cloud endpoint, so the code and docs are sent off the runner, scrubbed by the Prompt Scrubber and routed through the Private Inference Proxy rather than kept local. The privacy story is the operator's control over that endpoint and that path, not a claim that nothing leaves the runner. The prompt is published and inspectable. The local path stays available for an operator who runs a self hosted runner. Disabling DocsForest is disabling the workflow file; there is no NC hosted service in the loop.
...
- **A configured model endpoint, cloud by default.** Local first is a fully supported path on a self hosted runner; the out of the box default on hosted runners is a configured cloud model behind the scrubber and proxy. The project ships no model recommendation.
...
20. **Privacy posture.** Settled: the privacy philosophy rests on **open source, complete operator control, and first class local support**, not on a claim that data never leaves the runner. On cloud calls the Prompt Scrubber and Private Inference Proxy are the default composition and stay under the operator's control, consistent with "completely controllable by you". The principles section was rewritten to lead with these three pillars.
